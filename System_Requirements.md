...
- **Blinker for real-time board snapshots**: Introduce a `/api/boards/{boardId}/snapshot` endpoint and a `/api/boards/{boardId}/presence` helper so the server can offer a fallback when Firebase is unreachable. Emit `board_snapshot_ready` events through Socket.IO for clients that fail to connect to render quickly.
